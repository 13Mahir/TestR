...
CREATE TABLE schools (
  ...
  code      VARCHAR(10)  NOT NULL,   -- e.g. 'se', 'sm', 'ss'
  name      VARCHAR(100) NOT NULL,   -- e.g. 'School of Engineering'
...
CREATE TABLE branches (
  id         BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  code       VARCHAR(10)  NOT NULL,   -- e.g. 'CSE', 'ECE', 'MECH' (used in email)
...
CREATE TABLE student_profiles (
  user_id      BIGINT UNSIGNED PRIMARY KEY,   -- 1:1 with users
  batch_year   CHAR(2)         NOT NULL,       -- e.g. '22'
  branch_id    BIGINT UNSIGNED NOT NULL,
  roll_number  VARCHAR(10)     NOT NULL,       -- e.g. '001', '060'
...
CREATE TABLE active_sessions (
  ...
  access_token_jti    VARCHAR(64)  NOT NULL,   -- JWT ID claim of access token
  refresh_token_jti   VARCHAR(64)  NOT NULL,   -- JWT ID claim of refresh token
  ip_address          VARCHAR(45)  NOT NULL,   -- IPv4 or IPv6
  created_at          DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  expires_at          DATETIME NOT NULL,       -- matches refresh token expiry
  CONSTRAINT uq_session_user    UNIQUE (user_id),  -- enforces single session
...
CREATE TABLE password_reset_tokens (
  id               BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  user_id          BIGINT UNSIGNED NOT NULL,
  token            VARCHAR(128)    NOT NULL,     -- cryptographically random
  created_by       BIGINT UNSIGNED NOT NULL,     -- admin user id
...
CREATE TABLE courses (
  id          BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  course_code VARCHAR(15)  NOT NULL,   -- e.g. '22CS101T' — admin defined
  name        VARCHAR(100) NOT NULL,
  description TEXT,
  branch_id   BIGINT UNSIGNED NOT NULL,
  year        CHAR(2)      NOT NULL,   -- e.g. '22'
  mode        ENUM('T','P') NOT NULL,  -- T=Theory, P=Practical
  is_active   BOOLEAN      NOT NULL DEFAULT TRUE,
  created_by  BIGINT UNSIGNED NOT NULL,   -- admin user id
  created_at  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP
...
CREATE TABLE course_enrollments (
  ...
  enrolled_by  BIGINT UNSIGNED NOT NULL,   -- admin who enrolled
...
CREATE TABLE exams (
  id                      BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  course_id               BIGINT UNSIGNED NOT NULL,
  created_by              BIGINT UNSIGNED NOT NULL,   -- teacher user id
  title                   VARCHAR(200)    NOT NULL,
  description             TEXT,
  duration_minutes        INT UNSIGNED    NOT NULL,   -- overall timer
...
CREATE TABLE exam_attempts (
  ...
  status       ENUM(
                 ...
                 'auto_submitted'   -- timer expired, server submitted
...
CREATE TABLE subjective_grades (
  ...
  graded_by    BIGINT UNSIGNED NOT NULL,   -- teacher user id
...
CREATE TABLE proctor_violations (
  ...
  details        VARCHAR(255) NULL,   -- optional extra context
...
CREATE TABLE audit_logs (
  ...
  target_type  VARCHAR(50)     NOT NULL,   -- 'user', 'course', 'exam'
